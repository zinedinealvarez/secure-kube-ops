...
- name: Detect secrets with GitLeaks
  ...
  run: |
    ...
    docker run --rm \
      ...
      --config /repo/security-tools/gitleaks.toml \
      --report-format json \
...
Objetivo del control:
Analizar el código fuente para detectar patrones inseguros mediante análisis estático. Semgrep utiliza reglas automáticas adaptadas al contenido del repositorio y reglas locales versionadas en `security-tools/semgrep.yml`.

Criterio de paso:
Semgrep completa el análisis sin hallazgos que incumplan las reglas activas del proyecto.

Criterio de fallo:
Semgrep detecta hallazgos que incumplen las reglas automáticas seleccionadas o las reglas locales definidas en `security-tools/semgrep.yml`.
...
- name: Run Semgrep SAST
  run: docker run --rm -v "${{ github.workspace }}:/src" --workdir /src semgrep/semgrep semgrep scan --config auto --config security-tools/semgrep.yml . --error
```

El criterio de parada se configura mediante `--error`, que convierte los hallazgos de Semgrep en fallo del step. La configuración `--config auto --config security-tools/semgrep.yml` combina reglas obtenidas automáticamente con reglas locales versionadas en el repositorio.
