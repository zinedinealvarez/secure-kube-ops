...
Para esta fase se utiliza `kube-prometheus-stack` mediante Helm y un archivo `k8s/monitoring/values.yaml` versionado en el repositorio.
...
| Documento | Uso |
| --- | --- |
| `docs/observability/observability.md` | Instalación y comprobación de Prometheus, Grafana y Pushgateway. |
| `docs/deployment/cluster-portability.md` | Puesta en marcha completa de SecureKubeOps en otro clúster Kubernetes. |
| `docs/observability/pipeline-metrics-integration.md` | Envío y validación de métricas del pipeline mediante Pushgateway. |
| `docs/observability/pipeline-dashboard.md` | Diseño de paneles y consultas PromQL para Grafana. |
| `docs/ci-cd/pipeline-evidence.md` | Estructura de artifacts y origen de `reports/metrics.prom`. |
| `docs/security/runtime-security-monitoring.md` | Extensión de la observabilidad interna con inventario de workloads y reports de seguridad runtime mediante Trivy Operator. |
...
La seguridad runtime se documenta por separado en `docs/security/runtime-security-monitoring.md`. Esa fase reutiliza la base de Prometheus y Grafana, pero mantiene Trivy Operator en `k8s/runtime-security/` para separar observabilidad de seguridad sobre workloads en ejecución.
...
Las contraseñas reales quedan fuera de `README.md`, `docs/` y `k8s/monitoring/values.yaml`. El Secret se crea localmente en el clúster antes de instalar Helm. El archivo `k8s/monitoring/values.yaml` solo referencia el Secret y no contiene credenciales.
...
Este comando también aplica la configuración de persistencia definida en `k8s/monitoring/values.yaml`.
...
Pushgateway se despliega como servicio interno `ClusterIP` y queda monitorizado por Prometheus mediante el manifiesto `k8s/monitoring/pushgateway-servicemonitor.yaml`. La configuración completa del envío de métricas del pipeline se documenta en `docs/observability/pipeline-metrics-integration.md`.
...
3. Enviar una métrica de prueba o un archivo `metrics.prom` según `docs/observability/pipeline-metrics-integration.md`.
4. Abrir Prometheus mediante `kubectl port-forward`.
5. Consultar la métrica en Prometheus.
6. Abrir Grafana y comprobar los dashboards propios provisionados desde `k8s/monitoring/dashboards/grafana-dashboard-securekubeops-pipeline.yaml` y `k8s/monitoring/dashboards/grafana-dashboard-securekubeops-cluster-overview.yaml`.
...
- métricas del pipeline DevSecOps enviadas a Pushgateway desde el runner interno de AKS o, en validaciones locales, mediante `kubectl port-forward`.
...
- consulta de métricas `securekubeops_*` en Prometheus tras enviar un `metrics.prom` a Pushgateway con un `job` estable por workflow;
