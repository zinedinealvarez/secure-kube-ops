...
En esta configuración, `k8s/monitoring/pushgateway-values.yaml` usa:
...
El manifiesto `k8s/monitoring/pushgateway-servicemonitor.yaml` define:
...
Instalar primero `kube-prometheus-stack`, tal como se describe en `docs/observability/observability.md`.
...
Prometheus no recibe directamente el archivo `.prom`. El archivo se envía a Pushgateway y Prometheus lo obtiene al scrapear el Service de Pushgateway mediante `k8s/monitoring/pushgateway-servicemonitor.yaml`.
...
Los paneles definidos en `docs/observability/pipeline-dashboard.md` utilizan las métricas recibidas por Pushgateway y almacenadas por Prometheus.

El dashboard de Grafana se crea siguiendo las consultas y paneles definidos en `docs/observability/pipeline-dashboard.md`. La comprobación consiste en confirmar que las consultas PromQL devuelven datos en Prometheus y después usar esas mismas consultas en los paneles de Grafana.

## Orden completo de comprobación

El orden operativo para validar la integración es:

1. Instalar o actualizar `kube-prometheus-stack` con `docs/observability/observability.md`.
2. Instalar o actualizar Pushgateway con `k8s/monitoring/pushgateway-values.yaml`.
3. Aplicar `k8s/monitoring/pushgateway-servicemonitor.yaml`.
...
10. Usar `docs/observability/pipeline-dashboard.md` como guía para crear paneles.
...
- `docs/observability/observability.md` describe la instalación de Prometheus, Grafana y Pushgateway.
- `docs/deployment/cluster-portability.md` describe cómo reproducir esta configuración en otro clúster Kubernetes.
- `docs/ci-cd/pipeline-evidence.md` describe cómo los workflows generan `reports/metrics.prom`.
- `docs/observability/pipeline-dashboard.md` define las consultas PromQL y los paneles de Grafana.
- `k8s/monitoring/pushgateway-values.yaml` contiene la configuración Helm usada para desplegar Pushgateway.
- `k8s/monitoring/pushgateway-servicemonitor.yaml` contiene el `ServiceMonitor` que conecta Pushgateway con Prometheus.
