...
Cada artifact incluye `metadata.json`, `report.html` y los informes técnicos dentro de `tools/`. El resumen Markdown se muestra en GitHub Actions, mientras que `report.html` funciona como informe estático descargable de la ejecución. La evolución de esta estrategia y la estructura normalizada de los artifacts se documenta en `docs/pipeline-evidence.md`.
